...
## 2. 系统总览
- **硬件资源**：4 路 PWM 通道复用 LEDC 外设高分辨率定时器，GPIO7-10 为输出（默认关闭，输出低电平）。
- **软件组件**：
...
- **LEDC 配置**：
  ...
  - 全部通道共用一个定时器，默认频率 5 kHz；初始化时一次性完成 LEDC 通道配置，运行过程中仅通过占空比调节输出电平。
- **通道模式**：
  - `SWITCH`：0 = 关闭（0% 占空输出低电平），255 = 打开（100% 占空输出高电平）。
...
- **爆闪 (0x03)**：
  - 计算单次闪烁周期 = 总时长 / 点亮次数，ON/OFF 各占一半（50% 占空）。
  - 由软件定时器按周期在高/低占空之间切换，可在每个爆闪段后加入停顿阶段（参数 3，为毫秒）。
  - 爆闪结束后，恢复通道到指令触发前的占空比。
- **预设执行**：
...
- **控制指令结构**：
  ...
  ```
- **模式参数约定**：
  - 0x00（设置值）：1 字节目标值，0 表示关闭（GPIO 输出低电平），255 表示全开，其他值按 value/255 的占空比执行。
  - 0x01（渐变）：
    - Byte2：目标值（1 字节）。
