...
#### 开关状态
每路通道使用1字节无符号整数表示，0表示关闭（GPIO保持低电平），255表示开启（GPIO高电平），其他值表示占空比（value / 255）
...
|0x03|爆闪|第一个参数为1字节无符号整数，表示闪烁次数；第二个参数为2字节无符号整数，表示总闪烁时长，单位毫秒，闪烁周期为总时长/次数（50%占空，ON=周期/2，OFF=周期/2）；第三个参数为2字节无符号整数，表示停顿时长，单位毫秒，为0时表示完成一组闪烁就关闭|
